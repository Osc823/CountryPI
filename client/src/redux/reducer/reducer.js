  ...
  switch (action.type) {
    ...
    case SELECT_ACTIVITY:
      ...
      const actyFiltre =
        action.payload === "undefined"
          ? aa
          : aa.filter((ac) => ac.name === action.payload);
    ...
    case SELECT_SEASON:
      ...
      const seasonActy =
        action.payload === "All"
          ? ss
          : ss.filter((ss) => ss.season === action.payload);
      return {
        ...state,
    ...
    case PAGINATE:
      ...
      return {
        ...
        allCountries: state.countryFiltered.slice(
          startIndex,
          startIndex + ITEMS_PER_PAGE
        ),
        searchCountry: state.countryFiltered.slice(
          startIndex,
          startIndex + ITEMS_PER_PAGE
        ),
      };
    ...
    case PAGINATE_ACTIVITY:
      const nextActivityPage =
        action.payload === "next"
          ? state.activityCurrentPage + 1
          : state.activityCurrentPage - 1;
      const activityStartIndex = nextActivityPage * ITEMS_PER_PAGE;

      const maxActivityPage = Math.ceil(
        state.activityFiltered.length / ITEMS_PER_PAGE
      );
      ...
      return {
        ...
        allActivities: state.activityFiltered.slice(
          activityStartIndex,
          activityStartIndex + ITEMS_PER_PAGE
        ),
      };
    ...
    case ORDER_NAME_COUNTRY:
      ...
      if (action.payload === "az") {
        const countryName = [...state.countryFiltered].sort(
          (prev, next) => {
        ...
        return {
          ...state,
          allCountries: [...countryName].splice(0, ITEMS_PER_PAGE),
          countryFiltered: countryName,
          currentPage: 0,
        };
    ...
    case SELECT_CONTINENT:
      ...
      const contiFiltre =
        action.payload === "All"
          ? pp
          : pp.filter((co) => co.continent === action.payload);
    ...
    case ORDER_BY_POPULATION:
      if (action.payload === "minPopulation") {
        const allCountryNameOrder = [...state.countryFiltered].sort(
          (prev, next) => {
    ...
    case SEARCH_NAME:
      return {
        ...state,
        allCountries: [...action.payload].splice(0, ITEMS_PER_PAGE),
        searchCountry: [...action.payload].splice(0, ITEMS_PER_PAGE),
        currentPage: 0,
      };
    case ORDER_NAME_ACTIVITY:
      if (action.payload === "az") {
        const allActivityNameOrder = [...state.activityFiltered].sort(
          (prev, next) => {
            if (prev.name > next.name) return 1;
            if (prev.name < next.name) return -1;
            return 0;
      ...
      // eslint-disable-next-line no-fallthrough
      case ORDEN_AREA:
      if (action.payload === "AreaMen") {
        const areaCountry = [...state.allCountries].sort((prev, next) => {
        ...
        return{
          ...state,
          allCountries:[...areaCountry].splice(0, ITEMS_PER_PAGE),
          currentPage: 0,
        }
      }
      if (action.payload === "AreaMay") {
        const areaCountry = [...state.allCountries].sort((prev, next) => {
          if (prev.area > next.area ) return -1;
          if (prev.area < next.area) return 1
            return 0
